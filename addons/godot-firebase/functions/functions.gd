...
func execute(function: String, method: int, params: Dictionary = {}, body: Dictionary = {}) -> FunctionTask:
    ...
    function_task.connect("task_error", Callable(self, "_on_task_error"))
    function_task.connect("task_finished", Callable(self, "_on_task_finished"))
    function_task.connect("function_executed", Callable(self, "_on_function_executed"))
    ...
    if not body.is_empty():
        ...
        function_task._fields = JSON.new().stringify(body)
