...
func _on_FirebaseAuth_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	print_debug(test_json_conv.get_data()
	is_busy = false
	var res
	if response_code == 0:
...
func save_auth(auth: Dictionary) -> void:
	...
	if err != OK:
		...
		encrypted_file.store_line(JSON.new().stringify(auth))
...
func load_auth() -> void:
	...
	if err != OK:
		...
		var test_json_conv = JSON.new()
		test_json_conv.parse(encrypted_file.get_line())
		var encrypted_file_data = test_json_conv.get_data()
