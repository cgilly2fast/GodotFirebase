...
func _on_request_completed(result : int, response_code : int, headers : PackedStringArray, body : PackedByteArray) -> void:
	var bod
	if validate_json(body.get_string_from_utf8()).is_empty():
		var test_json_conv = JSON.new()
		test_json_conv.parse(body.get_string_from_utf8()).result
		bod = test_json_conv.get_data()
			...
			if offline:
				...
				if offline:
					save = {
						...
						var test_json_conv = JSON.new()
						test_json_conv.parse(_fields).result["fields"],
						"fields": test_json_conv.get_data()
			...
			if offline and file.file_exists(cache_path):
				if file.open_encrypted_with_pass(cache_path, File.READ, encrypt_key) == OK:
					...
					if content != "--deleted--":
						var test_json_conv = JSON.new()
						test_json_conv.parse(content).result
						body_return = test_json_conv.get_data()
				else:
				...
				for cache in cache_files:
					if file.open_encrypted_with_pass(cache, File.READ, encrypt_key) == OK:
						if file.get_line().begins_with(data[0]):
							var test_json_conv = JSON.new()
							test_json_conv.parse(file.get_line()).result)
							body_return.documents.append(test_json_conv.get_data()
					else:
